...
// bucketName returns the target bucket name
// this can be supplied via a command flag or will be
// generated by appending a random hash to the project name
func (s *S3Uploader) bucketName(projectName string) string {
	bucketName := s.Bucket

	if bucketName == "" {
		bucketName = fmt.Sprintf("%s-%s", projectName, someHash())
	}
...
func (s *S3Uploader) Upload(p *project.Project, name string, reader io.ReadSeeker, hash string) (string, string, error) {
	bucketName := s.bucketName(p.Name)
